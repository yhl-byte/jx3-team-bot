...
message_handler = on_regex(pattern=r"^猜测\s*(.*)$", priority=5) 
rules_command = on_regex(pattern=r"^海龟汤规则$", priority=5)
# message_handler = on_message(priority=99, block=False)
...
async def handle_rules(bot: Bot, event: GroupMessageEvent):
    ...
    1️⃣ 发送「开始海龟汤」开始游戏
    2️⃣ 玩家发送「报名海龟汤」参与游戏
    3️⃣ 玩家发送「结束海龟汤报名」开始竞选阶段
    4️⃣ 发送「竞选主持人」竞选主持人
    5️⃣ 主持人确认题目后开始游戏
    6️⃣ 玩家通过提问猜测真相以【猜测】开头
    ...
    await rules_command.send(rules_text.strip())